...
def main() -> None:
    ...
    st.caption("Sensitive values are masked before the prompt reaches the LLM and restored before the final report is shown.")
    ...
    if run_demo:
        ...
        st.subheader("Masked payload sent to the LLM")
        st.code(prompt_text, language="text")

        st.subheader("Final readable report")
        st.code(final_report, language="text")
        st.caption("The model only saw the masked prompt, and the readable report is restored before display.")
    else:
